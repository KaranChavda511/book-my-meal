...
export const register = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(201).json({ status: 'success',message:'User Register Successfully', data: result });
  } catch (err) {
    next(err);
  }
};
